...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addMatcher(
      ...
      (state, action) => {
        // ✅ On extrait le message de l'objet renvoyé par l'API
        // action.payload peut être { error: "..." } ou { message: "..." }
        const errorMessage =
          action.payload?.error || action.payload?.message || action.payload;

        state.error =
          typeof errorMessage === "string" ? errorMessage : "Server error";
      }
    );
  },
});
