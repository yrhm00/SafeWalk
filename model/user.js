export const readUser = async (SQLClient, { id }) => {
    const { rows } = await SQLClient.query('SELECT id, name, username, email, role, created_at FROM user WHERE id = $1', [id]);
    return rows[0];
};
...
export const createUser = async (SQLClient, { name = null, username, email, password_hash, role = 'citizen' }) => {
    const { rows } = await SQLClient.query('INSERT INTO user (name, username, email, password_hash, role) VALUES ($1, $2, $3, $4, $5) RETURNING id',
        [name, username, email, password_hash, role]);
    return rows[0]?.id ?? null;
};
export const deleteUser = async (SQLClient, { id }) => {
    const { rows } = await SQLClient.query('DELETE FROM user WHERE id = $1 RETURNING id', [id]);
};


export const readUserByUsername = async (SQLClient, { username }) => {
    if (!username) throw new Error('username manquant');
    const { rows } = await SQLClient.query('SELECT id, name, username, email, role, created_at FROM user WHERE username = $1', [username]);
    return rows[0] ?? null;
};
export const listuser = async (SQLClient, { limit = 100, offset = 0 } = {}) => {
    ...
    const query = 'SELECT id, name, username, email, role, created_at FROM user ORDER BY id DESC LIMIT $1 OFFSET $2';
